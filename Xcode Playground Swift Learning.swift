...
let success = Server.Result("6:00 am", "8:09 pm")
let failure = Server.Error("Out of cheese.")

switch success {
    case let .Result(sunrise, sunset):
        let serverResponse = "Sunrise is at \(sunrise) and sunset is at \(sunset)"
    case let .Error(error):
        ler serverResponse = "Failure... \(error)"
}
...
class SimpleClass: ExampleProtocol { //class don't need "mutating" keyword b/c methods can always modify the class
    ...
    func adjust() {
        simpleDecrtiption += " now 100% adjusted."
    }
}
